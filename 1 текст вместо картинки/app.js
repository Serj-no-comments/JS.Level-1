...
function handleClick(clickedButtonEvent) {
    ...
    const textProductName=card.button.innerText;
    
    if (textProductName === 'Подробнее') { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (textProductName==='Отмена') { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    const someText='Some words what we need to remove.'
